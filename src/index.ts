...
import { aiClassify } from "./ai-client.js";
...
async function handleOrganize(config: Config, dryRun: boolean): Promise<void> {
  ...
  const result = await organize(config, rules, classifyFn, dryRun);
...
async function handleWatch(config: Config): Promise<void> {
  ...
  process.once("SIGINT", () => { watcher.close(); process.exit(0); });
  process.once("SIGTERM", () => { watcher.close(); process.exit(0); });
  process.stdin.resume();
}

const program = createCLI(handleOrganize, handleWatch, getStatus);
program.parse();
